...
do
	ix.command.Add("DoorLock", {
		...
		OnRun = function(self, client)
			...
			if IsValid(entity) and entity:IsDoor() then
				...
				return "Pomyślnie zamknąłeś te drzwi."
			else
				return "@dNotValid"
			end
		end
	...
	ix.command.Add("DoorUnlock", {
		...
		OnRun = function(self, client)
			...
			if IsValid(entity) and entity:IsDoor() then
				...
				return "Udało Ci się otworzyć te drzwi."
			else
				return "@dNotValid"
			end
		end
	...
	ix.command.Add("DoorSetAccess", {
		description = "Przypisz dostęp graczowi (owner, tenant, guest) do drzwi, na które patrzysz.",
		privilege = "Manage Doors",
		adminOnly = true,
		...
		OnRun = function(self, client, target, acc)
			...
			if IsValid(entity) and entity:IsDoor() then
				...
				return "Pomyślnie zmieniłeś dostęp."
			else
				return "@dNotValid"
			end
		end
	...
	ix.command.Add("DoorRemoveAccess", {
		description = "Zabierz graczowi dostęp do drzwi, na które patrzysz.",
		privilege = "Manage Doors",
		adminOnly = true,
		...
		OnRun = function(self, client, target, acc)
			...
			if IsValid(entity) and entity:IsDoor() then
				...
				return "Pomyślnie uzyskałeś dostęp."
			else
				return "@dNotValid"
			end
		end
	...
	ix.command.Add("DoorResetAccess", {
		description = "Zresetuj dostęp do wszystkich drzwi.",
		privilege = "Manage Doors",
		adminOnly = true,
		OnRun = function(self, client, target, acc)
			local entity = client:GetEyeTrace().Entity
			...
			if IsValid(entity) and entity:IsDoor() then
				...
				return "Pomyślnie zresetowałeś dostęp."
			else
				return "@dNotValid"
			end
		end
