	...
	ix.command.Add("PKModeOn", {
		...
		OnRun = function(self, client)
			...
			if !state then
				...
				ix.chat.Send(nil, "notice", "Aktywowano tryb PK. Nie opuszczaj serwera do końca.")
				return
			end
			return "Tryb PK jest aktywny!"
		end
	})

	ix.command.Add("PKModeOff", {
		description = "Wyłącz tryb PK",
		privilege = "Manage PK Mode",
		adminOnly = true,
		OnRun = function(self, client)
			local state = GetNetVar("pkmode", false)
			if state then
				...
				ix.chat.Send(nil, "notice", "Tryb PK został wyłączony.")
				return
			end
			return "Tryb PK nie jest aktywny!"
		end
	})
end
...
if SERVER then
	function PLUGIN:CanPlayerUseCharacter(client)
		...
		if character and pkmode and !client:IsAdmin() then
			return false, "Nie można zmienić znaku, gdy tryb jest aktywny 'PK'"
		end
	end
	...
	local lp
	local clr, text = Color(225, 64, 64), "TRYB „PK” AKTYWNY"
