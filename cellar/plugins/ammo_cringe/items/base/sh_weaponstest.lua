...
if (CLIENT) then
	...
	function ITEM:PopulateTooltip(tooltip)
		...
		if self.hasLock then
			...
			lock:SetText("Istnieje zabezpieczenie przed nieuprawnionym użyciem typu biologicznego.")
		...
		if self.Info then
			if self.Info.Type then
				...
				skill:SetText(string.format("Typ: %s", Types[self.Info.Type]))
				skill:SetBackgroundColor(grayClr)
				skill:SizeToContents()
			end

			local skill = tooltip:AddRow("skill")
			skill:SetText(string.format("Umiejętność: %s", string.utf8lower(L((ix.skills.list[self.Info.Skill] or {}).name) or "nieznany")))
			skill:SetBackgroundColor(grayClr)
			skill:SizeToContents()

			if self.Info.Distance then
				local penalty = tooltip:AddRow("penalty")
				penalty:SetText("Premia za odległość:")
			...
			if self.Info.Dmg then
				...
				if self.Info.Dmg.Attack then
					...
					AP:SetIcon(icons[2])
					AP:SetText(string.format("%s ATAK", self.Info.Dmg.Attack))
					AP:SizeToContents()
				end
				...
				if self.Info.Dmg.AP then
					local AP = AddIconRow(tooltip)
					AP:SetIcon(icons[1])
					AP:SetText(string.format("%s PENETRACJA", self.Info.Dmg.AP))
					AP:SizeToContents()
				end
				...
				if self.Info.Dmg.Limb then
					...
					DMG:SetIcon(icons[3])
					DMG:SetText(string.format("%s OBRAŻENIA", self.Info.Dmg.Limb))
					DMG:SizeToContents()
				end
				...
				if self.Info.Dmg.Shock then
					local DMG = AddIconRow(tooltip)
					DMG:SetIcon(icons[4])
					DMG:SetText(string.format("%s — %s SZOK", self.Info.Dmg.Shock[1], self.Info.Dmg.Shock[2]))
					DMG:SizeToContents()
				end
				...
				if self.Info.Dmg.Blood then
					local DMG = AddIconRow(tooltip)
					DMG:SetIcon(icons[5])
					DMG:SetText(string.format("%s — %s KREW", self.Info.Dmg.Blood[1], self.Info.Dmg.Blood[2]))
					DMG:SizeToContents()
				end
				...
				if self.Info.Dmg.Bleed then
					...
					Bleed:SetText(string.format("%s%% SZANSA", self.Info.Dmg.Bleed))
...
ITEM.functions.awFix = {
	...
	OnRun = function(item)
		...
		client:SetAction("Naprawianie broni",repairtime,function()

			client:Notify("Pomyślnie naprawiłeś broń.")
