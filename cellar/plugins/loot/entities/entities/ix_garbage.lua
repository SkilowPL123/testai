...
if SERVER then
	...
	function ENT:OnSuccess(client)
		if math.Rand(0, 100) <= 50 then
			...
			ix.chat.Send(client, "it", "Nie znalazłeś niczego w śmieciach.", false, {client})
		end
	...
	function ENT:Use(client)
		...
		if !client:Crouching() then
			client:NotifyLocalized("Aby zacząć grzebać w śmieciach, musisz przykucnąć.")
			return
		end
		...
		timer.Create(uniqueID, 0.1, cleantime / 0.1, function()
			if (IsValid(client) and IsValid(self)) then
				...
				elseif !client:Crouching() then
					...
					self:OnFailed(client, "Aby zacząć grzebać w śmieciach, musisz przykucnąć.")
				...
				self:OnFailed(client, "Śmieci zostały usunięte, zanim można było je zebrać.")
			end
		end)
	end
else
