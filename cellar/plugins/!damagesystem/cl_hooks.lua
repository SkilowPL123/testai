...
do
	...
	net.Receive("ixCritUse", function()
		Derma_Query("Wykańczając tę ​​postać, zablokujesz ją na stałe, otrzymując cały ekwipunek zmarłego. Ten proces zajmie 15 sekund. Czy jesteś tego pewien?", "Wykończ postać", "Dobij", function() 
	...
	function PLUGIN:HUDPaint()
		...
		for but, _ in pairs(critPlayers) do
			...
			if IsValid(focus_ent) then
				...
				local text = string.format("KLIKNIJ [%s] ABY DODAĆ", use)
				local x = scrpos.x
				local y = scrpos.y + 16
