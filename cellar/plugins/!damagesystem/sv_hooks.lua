	...
	function PLAYER:SetCriticalState(state)
		...
		if state then
			...
			if self:IsCombine() then
				local letter = dispatch.AddWaypoint(self:GetShootPos(), "UTRATA BIO SYGNAŁU", "death", 30)
				Schema:AddCombineDisplayMessage(string.format("Mark %s: utracono sygnał biologiczny z jednostką naziemną!", letter), color_red)
			end
...
function PLUGIN:PostPlayerLoadout(client)
	...
	if !client.KilledByRP then
		...
		--character:SetBlood(math.max(character:GetBlood(), 5000))
		character:SetBlood(character:GetBlood())
...
function PLUGIN:PostPlayerDeath(client)
    local character = client:GetCharacter()
    if character then
        character:SetBleeding(false)  -- Wyłączenie krwawienia
        character:ResetLimbDamage()  -- Reset zdrowia kończyn
    end
end
...
do
	...
	ix.log.AddType("critKillStart", function(client, target)
		return string.format("%s próbuje wykończyć postać %s.", client:GetName(), target:GetName())
	end)

	ix.log.AddType("critStopped", function(client, target)
		return string.format("%s przestał wykańczać postać %s.", client:GetName(), target:GetName())
	end)

	ix.log.AddType("critKilled", function(client, target)
		return string.format("%s wykończył postać %s.", client:GetName(), target:GetName())
	end)
	...
	net.Receive("ixCritApply", function(len, client)
		...
		if state then
			...
			local character = client:GetCharacter()
			client:SetAction("Dobijasz postać...", 15)
...
net.Receive("ixCritUse", function(len, client)
	...
	if client:GetCharacter():GetLevel() < 3 then
		client:Notify("Niewystarczający poziom!")
		return
	end
	...
	if admins <= 0 then
		client:Notify("Na serwerze nie ma administratorów!")
		return
	end
