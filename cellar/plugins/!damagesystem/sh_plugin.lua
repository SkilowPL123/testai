...
do
	...
	ix.chat.Register("dmgMsg", {
		...
		OnChatAdd = function(self, speaker, text, bAnonymous, data)
			if data.t == 1 then
				chat.AddText(clrRed, string.format("Gracz %s (%s) cię wykańcza!", data.attacker:Name(), data.attacker:GetAnonID()))
			elseif data.t == 2 then
				chat.AddText(color_white, "Po śmierci w grze tracisz 30% swoich przedmiotów i tokenów.")
			elseif data.t == 3 then
				chat.AddText(color_white, "Przestali próbować cię wykończyć!")
			end
		end
	})
	...
	ix.chat.Register("dmgAdminMsg", {
		...
		OnChatAdd = function(self, speaker, text, bAnonymous, data)
			...
			if data.t == 1 then
				chat.AddText(clrRed, string.format("Gracz %s (%s) próbuje wykończyć gracza %s (%s)!", data.attacker:Name(), data.attacker:GetAnonID(), data.crit:Name(), data.crit:GetAnonID()))
			elseif data.t == 2 then
				chat.AddText(clrRed, string.format("%s (%s) został wykończony przez gracza %s (%s)!", data.crit:Name(), data.crit:GetAnonID(), data.attacker:Name(), data.attacker:GetAnonID()))
			end
		end
	})
end
