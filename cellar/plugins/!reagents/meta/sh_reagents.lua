...
if SERVER then
	...
	function PANEL:Init()
		...
		self.tab2:Dock(TOP)
			...
			self.tab2.label:SetText("Zawiera: ")
	...
	function PANEL:UpdateContainer(name, value, max, reagents)
		self.liquidText:SetText(string.format("%s (%i / %iml)", name, value, max))
		self.liquidText:SizeToContents()
			...
			if !self.reagentPanels[id] then
				...
				liquid.Think = function(self)
					if self.value != self.targetValue then
						...
						self:SetText(math.Round(self.value).."ml "..name)
					else
						self.Think = nil
					end
				end
