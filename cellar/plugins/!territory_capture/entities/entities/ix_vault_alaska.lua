...
if (SERVER) then
	...
	function ENT:Use( activator, caller )
		...
		if self:GetNetVar("reward_done") == true then
			...
			char:GetPlayer():NotifyLocalized("Czas na zdobycie łupów jeszcze nie nadszedł.")
		end
	end
	...
	function ENT:Draw()
		...
		if self:GetNetVar("nv_metro") == true then 
   			 metro_control = "POD KONTROLĄ" 
    		metro_col = Color( 0, 228, 34, 189) 
		else 
    		metro_control = "BRAK KONTROLI" 
   			 metro_col = Color( 214, 0, 0, 202) 
		end

		if self:GetNetVar("nv_otabridge") == true then 
			otabridge_control = "POD KONTROLĄ" 
			otabridge_col = Color( 0, 228, 34, 189) 
		else 
			otabridge_control = "BRAK KONTROLI" 
			otabridge_col = Color( 214, 0, 0, 202) 
		end

		if self:GetNetVar("nv_village") == true then 
			village_control = "POD KONTROLĄ" 
			village_col = Color( 0, 228, 34, 189) 
		else 
			village_control = "BRAK KONTROLI" 
			village_col = Color( 214, 0, 0, 202) 
		end

		if self:GetNetVar("nv_destroyedvillage") == true then 
			destroyedvillage_control = "POD KONTROLĄ" 
			destroyedvillage_col = Color( 0, 228, 34, 189) 
		else 
			destroyedvillage_control = "BRAK KONTROLI" 
			destroyedvillage_col = Color( 214, 0, 0, 202) 
		end

		if self:GetNetVar("nv_canalspit") == true then 
			canalspit_control = "POD KONTROLĄ" 
			canalspit_col = Color( 0, 228, 34, 189) 
		else 
			canalspit_control = "BRAK KONTROLI" 
			canalspit_col = Color( 214, 0, 0, 202) 
		end

		if self:GetNetVar("nv_fisherhouse") == true then 
			fisherhouse_control = "POD KONTROLĄ" 
			fisherhouse_col = Color( 0, 228, 34, 189) 
		else 
			fisherhouse_control = "BRAK KONTROLI" 
			fisherhouse_col = Color( 214, 0, 0, 202) 
		end

		if self:GetNetVar("nv_mines") == true then 
			mines_control = "POD KONTROLĄ" 
			mines_col = Color( 0, 228, 34, 189) 
		else 
			mines_control = "BRAK KONTROLI" 
			mines_col = Color( 214, 0, 0, 202) 
		end
		...
		cam.Start3D2D(fixedPos, fixedAng, 0.1)
			...
			draw.SimpleText( "Gotowość:", "Default", 50, 0, Color( 255, 255, 255, 155 ), TEXT_ALIGN_CENTER)
		...
		cam.Start3D2D(fixedPos_title, fixedAng, 0.1)
			draw.SimpleText( "Alaska", "Default", 50, 0, Color( 255, 255, 255, 155 ), TEXT_ALIGN_CENTER)
		cam.End3D2D()
		...
		cam.Start3D2D(fixedPos_metro, fixedAng, 0.1)
			draw.SimpleText( "METRO", "Default", 50, 0, Color( 255, 255, 255, 155 ), TEXT_ALIGN_CENTER)
		...
		cam.Start3D2D(fixedPos_otabridge, fixedAng, 0.1)
			draw.SimpleText( "AVANPOST OTA", "Default", 50, 0, Color( 255, 255, 255, 155 ), TEXT_ALIGN_CENTER)
		...
		cam.Start3D2D(fixedPos_village, fixedAng, 0.1)
			draw.SimpleText( "WIEŚ", "Default", 50, 0, Color( 255, 255, 255, 155 ), TEXT_ALIGN_CENTER)
		...
		cam.Start3D2D(fixedPos_destroyedvillage, fixedAng, 0.1)
			draw.SimpleText( "ZRUJNOWANA WIEŚ", "Default", 50, 0, Color( 255, 255, 255, 155 ), TEXT_ALIGN_CENTER)
		...
		cam.Start3D2D(fixedPos_canalspit, fixedAng, 0.1)
			draw.SimpleText( "KANAŁ", "Default", 50, 0, Color( 255, 255, 255, 155 ), TEXT_ALIGN_CENTER)
		...
		cam.Start3D2D(fixedPos_fisherhouse, fixedAng, 0.1)
			draw.SimpleText( "DOM RYBAKA", "Default", 50, 0, Color( 255, 255, 255, 155 ), TEXT_ALIGN_CENTER)
		...
		cam.Start3D2D(fixedPos_mines, fixedAng, 0.1)
			draw.SimpleText( "KOPALNIE", "Default", 50, 0, Color( 255, 255, 255, 155 ), TEXT_ALIGN_CENTER)
