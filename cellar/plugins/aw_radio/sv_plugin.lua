...
function PLUGIN:RegisterKasseta(client, uniqueID, name, path, desc,duration)
	...
	if !client:IsAdmin() then return "Nie jesteś administratorem!" end
	...
	if file.Read(path,"GAME") then
		...
		return "Kaseta "..name.." pomyślnie utworzona."

	end

	client:PrintMessage(HUD_PRINTCONSOLE,"Musisz określić ścieżkę z folderu źródłowego garrysmod\nPrzykład:\n	sound/music/HL2_song23_SuitSong3.mp3")

	return "Określono nieprawidłową ścieżkę! Sprawdź swoją konsolę."
...
function PLUGIN:EjectKasseta(radio)
	...
	if kasseta then
		...
		client:Notify("Pomyślnie wyjęłeś kasetę.")
    ...
    if id then
		...
		if IsValid(client) then
			...
			if kasseta and kasseta.IsKasseta then
				...
				client:Notify("Pomyślnie włożyłeś kasetę.")
	...
	if IsValid(ent) then
		...
		if !kasseta then 
			...
			if enable then
				...
				(isent and ent:GetOwner() or radio.player):Notify("W radiu nie ma kasety!")
...
function PLUGIN:SWEPHandle(weapon,button)
	...
	client:Notify("Proszę poczekać przed następną akcją!")
