...
ix.command.Add("CreateKasseta", {
	description = "[UniqueID] && [Nazwa] && [Ścieżka] && [Opis] && [Czas trwania]",
	arguments = {
		ix.type.string,
		ix.type.string,
        ix.type.string,
...
ix.command.Add("GetKassets", {
	description = "Pobierz wszystkie zarejestrowane taśmy na serwer",
...
if CLIENT then
    ...
    function PLUGIN:HUDPaint()
        ...
        if IsValid(weapon) and weapon:GetClass() == "aw_radio" then
            ...
            ix.util.DrawText("Start/Pauza – LPM", ScrW()*.5, ScrH()*.86, nil, 1, 1, "ixBigFont")

            ix.util.DrawText("Włóż/wyjmij kasetę – RMB", ScrW()*.5, ScrH()*.89, nil, 1, 1, "ixBigFont")

            ix.util.DrawText("Powrót do góry – R", ScrW()*.5, ScrH()*.92, nil, 1, 1, "ixBigFont")
