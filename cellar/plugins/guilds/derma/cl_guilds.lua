...
function PANEL:show_guild()
  ...
  self.label_level:SetText('Poziom gildii: ')
  ...
  self.current_level:SetText(level < 1 and 'nieobecny' or ('/ '):rep(level))
  ...
  if level > 0 then
    ...
    self.balance:SetText('Balansować: '..self.guild:get_balance())
  ...
  self:add_navigation_button('Lista członków', function()
    ...
    net.Receive('ix_guild_members_receive', function(length, client)
      ...
      for i = 1, #uncompressed do
        ...
        role:SetText('Rola: '..uncompressed[i].guild_data.role)
        role:SizeToContents()
        ...
        if uncompressed[i].guild_data.group then
          ...
          group:SetText('Grupa: '..uncompressed[i].guild_data.group)
  ...
  self:add_navigation_button('Ustawienia', function()
    ...
    roles_tree.OnMousePressed = function(this, key)
      ...
      derma_menu:AddOption('Dodaj rolę', function()
        Derma_StringRequest('Stwórz rolę', 'Wprowadź nazwę roli', 'Nowa rola', function(text)
    ...
    groups_tree.OnMousePressed = function(this, key)
      ...
      local derma_menu = DermaMenu()
      derma_menu:AddOption('Dodaj grupę', function()
        Derma_StringRequest('Stwórz grupę', 'Wprowadź nazwę grupy', 'Nowa grupa', function(text)
    ...
    for k, v in pairs(self.guild:get_roles()) do
      name = roles_tree:AddNode(v.no_edit and k..' (niezmienny)' or k)
      ...
      name.DoRightClick = function(this, node)
        ...
        local derma_menu = DermaMenu()
        local privileges = derma_menu:AddSubMenu('Przywilej')
          ...
          privileges:AddOption(p, function()
            ...
            privilege.DoRightClick = function(this2)
              local remove_privilege = DermaMenu()
                remove_privilege:AddOption('Usuń '..GUILD_ENUM_PRIVILEGES_TO_NAME[i], function()
                  this2:Remove()
  ...
  self:add_navigation_button('Zaproszenia', function()
  end)

  self:add_navigation_button('Usuń ułamek', function()
  end)
end
...
function PANEL:create_new_guild()
  ...
  title:SetText('Tworzenie frakcji')
  title:SizeToContents()
  title:Center()

  local name_tip = self:Add('DLabel')
  name_tip:SetFont('ixMenuButtonFontSmall')
  name_tip:SetText('Tytuł musi zawierać co najmniej '..ix.config.Get('minGuildNameLength')..' i maksymalnie '..ix.config.Get('maxGuildNameLength')..' liter(ов)')
  ...
  name:SetPlaceholderText('Wprowadź nazwę')

  local apply = self:Add('DButton')
  apply:SetFont('ixSubTitleFont')
  apply:SetText('Przyjmij')
...
function PANEL:check_invites(invites)
  ...
  for i = 1, #invites do
    ...
    invited_by:SetText('Zaproszony: '..invites[i].invited_by)
