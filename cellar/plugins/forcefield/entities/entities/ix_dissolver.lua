	...
	function ENT:Draw()
		...
		if self:GetToggle() then
			...
			if self.alpha > 0 then
				draw.SimpleText("OBYWATEL, NIE MA DOSTĘPU", "cp_hud4", (self:GetPos():Distance(dummy:GetPos()) * 16.5) / 2, 2120, Color(255, 255, 255, self.alpha), TEXT_ALIGN_CENTER, 1)
			end

			draw.SimpleText("UWAGA", "cp_hud4", (self:GetPos():Distance(dummy:GetPos()) * 16.5) / 2, 1550, Color(255, 255, 255, math.abs(math.sin(CurTime())) * 200), TEXT_ALIGN_CENTER, 1)
			draw.SimpleText("LOKALIZACJA JEST OGRANICZONA", "cp_hud4", (self:GetPos():Distance(dummy:GetPos()) * 16.5) / 2, 1750, Color(255, 255, 255, math.random(150, 255)), TEXT_ALIGN_CENTER, 1)
			cam.End3D2D()
