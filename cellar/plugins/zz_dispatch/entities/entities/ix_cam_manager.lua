...
function ENT:OnDeath(camera, client)
	local letter = dispatch.AddWaypoint(camera:GetPos(), "KAMERA ZNISZCZONA", "warn", 60)
		
	Schema:AddCombineDisplayMessage(string.format("Mark %s: utracono sygnał kamery!", letter), color_red)
...
function ENT:ShouldHate(client)
	if client:IsNPC() then
		local alarm, alarmtext, alarmtext2 = false, "Etykieta %s:", ""

		if client:Classify() == CLASS_HEADCRAB then
			alarmtext = alarmtext.." inwazja pasożytów!"
			alarmtext2 = "Pasożyty"
			alarm = true
		elseif client:Classify() == CLASS_ANTLION then
			alarmtext = alarmtext.." Inwazja antlionów!"
			alarmtext2 = "Antiliony"
			alarm = true
		elseif client:Classify() == CLASS_ZOMBIE then
			alarmtext = alarmtext.."nekrotyczna inwazja!"
			alarmtext2 = "Zombie"
			alarm = true
		end
	...
	if client:Team() == FACTION_VORTIGAUNT then
		if client:HasWeapon("ix_vortbeam") then
			return true, "Mark %s: niekontrolowany biotyk!", "Niekontrolowany biotyk", "hazard", color_red
		end
	...
	elseif !client:IsCombine() and !client:IsCityAdmin() then
		...
		for _, weapon in pairs(client:GetWeapons()) do
			if IsValid(weapon) and weapon.ixItem then
				return true, "Etykieta %s: "..(card and "uzbrojony" or "niezidentyfikowany uzbrojony ").." 647E! "..textData, "94B 647E "..textData, "gun", color_red
			end
		end
		...
		if card then
			...
			return true, "Tag %s: Wykryto niezidentyfikowaną osobę!", "Niezidentyfikowana osoba", "poi", yellow
		end

		if datafile and data then
			if data.status == "Anti-Citizen" then
				return true, "Tag %s: wykryto anty-obywatela! "..textData, "Antyobywatel "..textData, "warn", color_red
			elseif data.bol then
				return true, "Tag %s: nadzór "..textData, textData, "poi", yellow
			end
		end
	end
