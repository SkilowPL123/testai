...
dispatch.WorldAction({
	LabelOn = "Wyłącz racje",
	LabelOff = "Włącz racje",
	--Icon = "",
	Order = 2,
...
dispatch.WorldAction({
	Label = "Oznacz jako Anty-Obywatela",
	...
	Receive = function(self, client)
		...
		local letter = dispatch.AddWaypoint(entity:GetShootPos(), "Anty-Obywatel"..data, "warn", 60)

		Schema:AddCombineDisplayMessage(string.format("Etykieta %s: %s", letter, card and ("obywatel"..data.." oznaczony jako intruz!") or "Niezidentyfikowana osoba została oznaczona jako intruz!"), color_red)
	end
})

dispatch.WorldAction({
	Label = "Otwórz plik osobisty",
...
dispatch.WorldAction({
	LabelOn = "Odblokuj drzwi",
	LabelOff = "Zamknij drzwi",
	--Icon = "",
	Order = 3,
...
dispatch.WorldAction({
	LabelOn = "Włącz blokadę",
	LabelOff = "Wyłącz blokadę",
	--Icon = "",
	Order = 2,
...
dispatch.WorldAction({
	LabelOn = "Odblokuj zamek",
	LabelOff = "Zablokuj zamek",
	--Icon = "",
	Order = 3,
...
dispatch.WorldAction({
	...
	Action = function(self, entity, client, trace)
		...
		m:AddOption("Zezwól na dostęp każdemu", function()
			self:MsgStart()
				net.WriteEntity(entity)
				net.WriteUInt(0, 3)
			self:MsgEnd()
		end)
		m:AddOption("Wejście wyłącznie za okazaniem dostępu", function()
			self:MsgStart()
				net.WriteEntity(entity)
				net.WriteUInt(1, 3)
			self:MsgEnd()
		end)
		m:AddOption("Przejście niedostępne", function()
			self:MsgStart()
				net.WriteEntity(entity)
				net.WriteUInt(2, 3)
			self:MsgEnd()
