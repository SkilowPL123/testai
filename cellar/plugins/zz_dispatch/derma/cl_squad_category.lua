...
function PANEL:SetupSquadFull(squad)
	...
	self.squad = squad
	self.data = {
		name = self.isStatic and "BEZ PT" or "PT "..squad:GetTagName(),
...
function PANEL:OpenMenu(squad)
	...
	if isDispatch or isLeader then
		menu:AddOption("Rozwiąż", function() 
	...
	if isDispatch then
		menu:AddOption("Nalicz system operacyjny całej grupie", function() 
			Derma_StringRequest(
				squad:GetTagName(), 
				"Wprowadź żądaną liczbę punktów sterylizacji, które mają zostać przyznane KAŻDEMU członkowi zespołu patrolowego (dodatnia lub ujemna)",
				"3",
				function(value) 
