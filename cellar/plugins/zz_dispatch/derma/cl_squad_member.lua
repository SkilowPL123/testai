...
function PANEL:Init()
	...
	self.hp:SetText("100%")
	self.code_text:SetText("KOD 1")
	self.area_text:SetText("Nie dotyczy")
...
function PANEL:OpenMenu(target)
	...
	if isDispatch then
		...
		move:AddOption("Nowa grupa", function()
			net.Start("squad.menu.move")
				net.WriteUInt(id, 32)
				net.WriteBool(true)
			net.SendToServer()
		...
		if isLeader then
			squad:AddOption("Wyrzuć", function()
				net.Start("squad.menu.move")
					net.WriteUInt(id, 32)
					net.WriteBool(true)
				net.SendToServer()
	...
	if isDispatch then
		menu:AddOption("Obserwuj", function() 
			net.Start("squad.menu.spectate")
				net.WriteUInt(id, 32)
			net.SendToServer()
		end):SetImage("icon16/zoom.png")

		menu:AddOption("Nalicz system operacyjny", function() 
			Derma_StringRequest(
				target:GetName(), 
				"Wprowadź żądaną liczbę punktów sterylizacji do wydania (dodatnią lub ujemną)",
				"3",
				function(value) 
	...
	menu:AddSpacer()
	menu:AddOption("Otwórz plik osobisty", function() 
