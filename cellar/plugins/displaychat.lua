...
ix.option.Add("Wyświetl tekst nad głową", ix.type.bool, true, {
	category = "Speech Display"
})

ix.option.Add("Limit długości wiadomości", ix.type.number, 256, {
	category = "Speech Display",
	min = 10, max = 2048
})

ix.option.Add("Czas trwania na znak", ix.type.number, 0.4, {
	category = "Speech Display",
...
function PLUGIN:MessageReceived(client, messageInfo)
	if (IsValid(client) and client != LocalPlayer() and ix.option.Get("Wyświetl tekst nad głową", false)) then
		if (hook.Run("ShouldChatMessageDisplay", client, messageInfo) != false) then
			local maxLen = ix.option.Get("Limit długości wiadomości")
			local text = messageInfo.text
			local textLen = string.utf8len(text)
			local duration = math.max(2, math.min(textLen, maxLen) * ix.option.Get("Czas trwania na znak"))

			stored[client] = {
				text = textLen > maxLen and utf8.sub(text, 1, ix.option.Get("Limit długości wiadomości")).."..." or text,
...
function PLUGIN:HUDPaint()
	if (ix.option.Get("Wyświetl tekst nad głową", false) and stored and !table.IsEmpty(stored)) then
