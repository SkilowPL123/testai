			...
			if (value != nil) then
				if (istable(value)) then
					...
					if (count < defaulSpecialPoints) then
						return false, "Musisz wykorzystać wszystkie punkty SPECIAL!"
					end
...
if SERVER then
	...
	local function OpenBackward()
		...
		label:SetText("Ponowny podział punktów")
		...
		label:SetWrap(true)
		label:SetText("Ze względu na aktualizację cech i umiejętności, musisz ponownie rozdysponować punkty cech postaci.")
		label:SizeToContents()
		label:Dock(TOP)
	...
	local function OpenLevelUp()
		...
		label:SetFont("ixMenuButtonHugeFont")
		label:SetTextColor(ix.config.Get("color"))
		label:SetText("Podnieść do właściwego poziomu")
		label:SizeToContents()
		label:Dock(TOP)
		label:DockMargin(10, 0, 0, 0)
		...
		label:SetAutoStretchVertical(true)
		label:SetWrap(true)
		label:SetText("Twój poziom wzrósł! Rozkładać"..pointsmax.." charakterystyczne punkty.")
		label:SizeToContents()
		label:Dock(TOP)
		label:DockMargin(0, 0, 0, 0)
