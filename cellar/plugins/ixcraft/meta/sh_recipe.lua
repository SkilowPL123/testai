...
function RECIPE:OnCanCraft(client)
	...
	if self.station and IsValid(client.ixStation) and client.ixStation:GetStationTable().uniqueID != self.station then
		...
		return false, "Należy użyć %s!", stationInfo.GetName and stationInfo:GetName() or L(stationInfo.name)
	end
		...
		if skillTable then
			...
			if ourSkill < needed then
				if IsValid(client) then
					return false, "Wymagana umiejętność %s!", string.format("%s %s", L(skillTable.name, client), needed)
				else
					return false, "Wymagana umiejętność %s!", string.format("%s %s", L(skillTable.name), needed)
				end
			end
		end
	end
	...
	if (bHasItems == false) then
		return false, "Brak wymaganych elementów: %s", missing
	end
	...
	if (bHasTools == false) then
		return false, "Brak niezbędnych narzędzi %s", missing
	end
...
if (SERVER) then
	function RECIPE:OnCraft(client)
		...
		return true, "Pomyślnie stworzyłeś %s!", self.GetName and self:GetName() or self.name
	end
end
