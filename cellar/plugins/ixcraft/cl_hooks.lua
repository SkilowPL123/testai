...
function PLUGIN:PopulateRecipeTooltip(tooltip, recipe)
	...
	if recipe.station then
		...
		if stationInfo then
			...
			station:SetText(string.format("- %s %s", "Zbyt daleko od stacji", stationInfo.GetName and stationInfo:GetName() or L(stationInfo.name)))
	...
	if istable(recipe.skill) then
		...
		if skillTable then
			...
			s:SetText(string.format("- %s %s %s", "Umiejętność", skillName, value))
			s:SizeToContents()
		end
	end

	local result = tooltip:AddRow("result")
	result:SetText("Tworzy")
	...
	if istable(recipe.skill) then
		...
		result:SetText("- +"..string.format("%i doświadczenie w umiejętnościach %s", xp * intFactor, skillName))
		result:SizeToContents()
	end
