...
ix.chat.Register("level", {
	...
	OnChatAdd = function(self, speaker, text, bAnonymous, data)
		if data.t == 1 then
			chat.AddText(color_white, string.format("Twój poziom wzrósł do %s! Wpisz /lvl, aby pozbyć się obecnych punków.", LocalPlayer():GetCharacter():GetLevel()))
		elseif data.t == 2 then
			local name = L((ix.skills.list[data.skill] or {}).name) or "Unknown"

			chat.AddText(color_white, string.format("Twoja umiejętność %s została ulepszona do %s!", name, data.value))
		elseif data.t == 3 then
			chat.AddText(color_white, string.format("Twój poziom został obniżony do %s!", LocalPlayer():GetCharacter():GetLevel()))
		elseif data.t == 4 then
			local name = L((ix.skills.list[data.skill] or {}).name) or "Unknown"

			chat.AddText(color_white, string.format("Twoja umiejętność %s została obniżona do %s!", name, data.value))
		end
	end
})
...
ix.command.Add("CharSetLevel", {
	...
	OnRun = function(self, client, target, targetValue)
		...
		return "Pomyślnie zmieniłeś poziom swojej postaci."
	end
})
...
ix.command.Add("CharAddLevel", {
	...
	OnRun = function(self, client, target, targetValue)
		...
		return "Pomyślnie dodałeś poziomy do swojej postaci."
	end
})
...
ix.command.Add("CharAddLevelXP", {
	...
	OnRun = function(self, client, target, xp)
		...
		return "Udało Ci się zdobyć doświadczenie dla swojej postaci."
	end
})