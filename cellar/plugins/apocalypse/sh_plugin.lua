	...
	function PLUGIN:AdvanceDisease(character)
		...
		if stage == 1 then
			timer.Simple(40, function()
				ix.chat.Send(character:GetPlayer(), "me", "zaczyna mimowolnie kaszleć.")
				timer.Simple(40, function()
					ix.chat.Send(character:GetPlayer(), "me", "drży i rozgląda się nerwowo, szczękając zębami.")
				end)
			end)
		elseif stage == 2 then
			timer.Simple(40, function()
				ix.chat.Send(character:GetPlayer(), "me", "kaszle krwią i ciężko oddycha.")
			end)

		elseif stage == 3 then
			ix.chat.Send(character:GetPlayer(), "me", "traci rozum i wpada w szał.")
